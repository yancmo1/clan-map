...
# Database path - direct path since app runs on Pi
BOT_DB_PATH = '/home/pi/coc-discord-bot/synced-data/cwl_data.db'

def get_bot_db_connection():
    """Get connection to the bot's database (local on Pi)"""
    try:
        if os.path.exists(BOT_DB_PATH):
            return sqlite3.connect(BOT_DB_PATH)
        else:
            print(f"Database not found at {BOT_DB_PATH}")
            return None
...
def load_clan_data():
    """Load clan data from bot database on Pi"""
    conn = get_bot_db_connection()
    if not conn:
        print("Failed to connect to Pi database")
        return []
...
def save_clan_data(data):
    """Save clan data to bot database (local on Pi)"""
    conn = get_bot_db_connection()
    if not conn:
...
def geocode_location(location):
    """Convert location string to coordinates using a free geocoding service"""
    try:
...
def submit_location():
    ...
    if conn:
        cursor = conn.cursor()
        cursor.execute('SELECT role FROM players WHERE name = ? COLLATE NOCASE', (name,))
...
def admin_set_role(name, role):
    ...
    if conn:
        cursor = conn.cursor()
        cursor.execute('UPDATE players SET role = ? WHERE name = ? COLLATE NOCASE', (role, name))
        if cursor.rowcount > 0:
            conn.commit()
            conn.close()
